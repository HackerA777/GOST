...
static constexpr uint8_t sTable4x256[4][256] = {
...
static constexpr unsigned int tTable[8][16] = {
...
halfVectorMagma xOR(const halfVectorMagma& src1, const halfVectorMagma& src2)
{
	uint32_t tmp = src1.vector ^ src2.vector;
	return halfVectorMagma(tmp);
}

halfVectorMagma mod32(const halfVectorMagma& src, const halfVectorMagma& key)
{
	uint32_t tmp = src.vector + key.vector;
	return halfVectorMagma(tmp);
}

halfVectorMagma transformationT(const halfVectorMagma& src)
{
	halfVectorMagma result;
...
void expandKeys(const key& key, halfVectorMagma(&roundKeys)[32])
{
	int q = 0;
	for (int i = 0; i < 3; i++) {
		int step = 0;
...
halfVectorMagma gTransformation(const halfVectorMagma& key, const halfVectorMagma& half)
{
	halfVectorMagma tmp = mod32(half, key);
	tmp = transformationT(tmp);
	tmp.vector = (tmp.vector << 11) | (tmp.vector >> 21);
	return tmp;
}

byteVectorMagma transformationG(const byteVectorMagma& src, const halfVectorMagma& key)
{
	halfVectorMagma gResult = gTransformation(key, src.halfsData.lo.vector);
	halfVectorMagma tmp = xOR(gResult, src.halfsData.hi.vector);
	return byteVectorMagma(tmp, src.halfsData.lo.vector);
}

byteVectorMagma encryptBlock(const byteVectorMagma& src, const halfVectorMagma(&roundKeys)[32])
{
	byteVectorMagma tmp = src;
	for (int i = 0; i < 31; i++) {
		tmp = transformationG(tmp, roundKeys[i]);
	}
	halfVectorMagma gResult = gTransformation(roundKeys[31], tmp.halfsData.lo.vector);
	halfVectorMagma tmp2 = xOR(gResult, tmp.halfsData.hi.vector);
	return byteVectorMagma(tmp.halfsData.lo.vector, tmp2);
}

void Magma::processData(std::span<const byteVectorMagma> src, std::span<byteVectorMagma> dest) const
{
	#pragma omp parallel for
	for (int i = 0; i < src.size(); ++i)
	{
		dest[i] = encryptBlock(src[i], this->roundKeys);
	}
}
