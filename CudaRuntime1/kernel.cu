...
__device__ const uint8_t table4x256[4][256] = {
...
__device__ magma::half_block_t magma::invert(const half_block_t block) {
    half_block_t result;
    for (int i = 0; i < 4; ++i) {
...
__device__ uint32_t magma::Add(const half_block_t a, const half_block_t b) {
    return a.uint ^ b.uint;
}

__device__ uint32_t magma::Add32(const half_block_t a, const half_block_t b) {
    return a.uint + b.uint;
}
...
//}
__device__ magma::half_block_t magma::magma_T(const half_block_t data) {
    half_block_t result;
...
__device__ magma::half_block_t magma::magma_g(const key_t round_key, const half_block_t block) {
...
__device__ magma::block_t magma::magma_G(const key_t round_key, const block_t block) {
...
__device__ magma::block_t magma::magma_G_last(const key_t round_key, const block_t block){
    block_t result = block;
    result.lo = magma_g(round_key, result.lo);
    result.lo.uint = Add(result.lo, block.hi);
...
__global__ void magma::Encrypt(const key_set& round_key, const block_t& block, block_t& result_block)
{
...
__global__ void magma::Decrypt(const key_set& round_key, const block_t& block, block_t& result_block)
{
    block_t result;
    result = magma_G(round_key.keys[0], block);
    for (int i = 1; i < 8; ++i)
...
int main()
{
    ...
    decryptCuda(resultEncrypt.bytes, resultDecrypt.bytes, 8, keys);
...
void encryptCuda(const uint8_t* block, uint8_t* out_block, const size_t data_size, const key_set& keys) {
