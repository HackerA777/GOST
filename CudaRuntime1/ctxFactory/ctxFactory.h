...
struct OsslCtx{
    ...
    int decrypt(unsigned char* data, unsigned char* out, int size){
        int result = 0;
        check(EVP_DecryptUpdate(ctx, out, &result, data, size), "decrypt");
        return result;
    }

    int final_decrypt(unsigned char* data){
        int result = 0;
        check(EVP_DecryptFinal(ctx, data, &result), "decryptf");
        return result;
    }
};
...
public:
    ...
    OsslCtx encryptInit(unsigned char* key){
        auto ctx= check(EVP_CIPHER_CTX_new(), "ctx new");
        check(EVP_EncryptInit(ctx, cipher, key, nullptr), "Enc init");
        return OsslCtx(ctx);
    }

    OsslCtx decryptInit(unsigned char* key){
        auto ctx= check(EVP_CIPHER_CTX_new(), "ctx new");
        check(EVP_DecryptInit(ctx, cipher, key, nullptr), "Dec init");
        return OsslCtx(ctx);
    }
