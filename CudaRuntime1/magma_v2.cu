...
//}
//
//void magma_v2::encryptCuda(cuda_ptr<magmaBlockT[]> blocks, cuda_ptr<magmaBlockT[]> out_blocks, const size_t countBlocks) {
//    
...
//
//    cudaCheck(cudaMemcpyAsync(dev_keys.get(), keys.keys, sizeof(magmaKeySet), cudaMemcpyHostToDevice));
//
//    //cuda_ptr<magmaBlockT[]> dev_blocks = cuda_alloc_async<magmaBlockT[]>(buffSize);
//
//    //cudaCheck(cudaHostRegister((void*)data, buffSize, cudaHostRegisterDefault));
//   
//    for (auto& stream : streams) { //for (auto i=0; i < countBlocks; i+=streams_buffsize){ cudaCheck(CudaGetLastError() }
//
//        //cudaCheck(cudaMemcpyAsync(dev_blocks.get(), blocks, buffSize, cudaMemcpyHostToDevice, stream[i]));
//        //cudaCheck(cudaGetLastError());
//
//        encrypt <<< blockSize, gridSize, 0, streams[i] >>> (*dev_keys, blocks.get(), countBlocks);
//
//        if (blocks != out_blocks)
//            cudaCheck(cudaHostRegister((void*)out_blocks, countBlocks * sizeof(magmaBlockT), cudaHostRegisterDefault));
//
//        cudaCheck(cudaMemcpyAsync(out_blocks, blocks.get(), countBlocks * sizeof(magmaBlockT), cudaMemcpyDeviceToHost));
//
//        
//    }
//    if (blocks != out_blocks)
...
//}
//
//void callKernelEncrypt(const uint8_t* blocks, uint8_t* out_blocks, const magmaKeySet inputKeys, const size_t buffSize, cudaStream_t stream) { //перенести загрузку ключей в конструктор
//   
//    encrypt << < blockSize, gridSize, 0, stream >> > (*dev_keys, dev_blocks.get(), countBlocks);
//
//    cudaCheck(cudaGetLastError());
//
//    if (blocks != out_blocks)
//        cudaCheck(cudaHostRegister((void*)out_blocks, buffSize, cudaHostRegisterDefault));
//
//    cudaCheck(cudaMemcpyAsync(out_blocks, dev_blocks.get(), buffSize, cudaMemcpyDeviceToHost, stream));
//
//    if (blocks != out_blocks)
//        cudaCheck(cudaHostUnregister((void*)blocks));
//
...
//}
//
//void magma_v2::decryptCuda(const uint8_t* blocks, uint8_t* out_blocks, const magmaKeySet inputKeys, const size_t countBlocks) {
//    cuda_ptr<magmaKeySet> dev_keys = cuda_alloc<magmaKeySet>();
...
//
//    decryptCuda(resultBlock.bytes, resultBlock.bytes, testKeys, 1);
...
//}
//
//double magma_v2::testSpeedUnequalBytes() {
...
//}
//
//void magma_v2::searchBestBlockAndGridSize() {
//    cudaDeviceProp prop;
...
//}
//
//magma_v2::magma_v2(const unsigned char keys[32], const size_t buffSize) {
//    copyKeys(keys);
//    this->buffSize = buffSize;
//    if (buffSize >= 16 * 8) {
//        this->streamsCount = buffSize % (16 * 8);
//        this->streams.resize(streamsCount);
//        this->ptrs.resize(streamsCount);
//    }
//}
//
//magma_v2::~magma_v2() {
